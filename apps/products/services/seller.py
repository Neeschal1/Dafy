...
def seller_detail(seller):
    ...
    details = [salesman_name, salesman_address, seller.validated_data['Selling_Price'], seller.validated_data['Estimated_Bought_Date']]
    
    cache.set(f"seller_detail_{cache_key}", details, timeout=300)
    
    return Response({"Message":{"Details":{"Initial Details":cache.get(cache_key), "Images Details":cache.get(f"images_{cache_key}"), "Sellers Detail":cache.get(f"seller_detail_{cache_key}")}}})